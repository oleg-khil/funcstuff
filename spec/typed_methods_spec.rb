...
describe "typed_methods" do
  ...
  describe "def_typed" do
    describe "regular typed method" do
      ...
      it "must succeed if types are correct" do
        _(instance.__send__(method_name, 1, 2.0)).must_equal 1 + 2.0
      end

      it "must raise if types are incorrect" do
        _ { instance.__send__(method_name, 1, 2.0.to_s) }.must_raise TypeError
      end

      it "must raise if arguments count is wrong" do
        _ { instance.__send__(method_name, 1) }.must_raise ArgumentError
        _ { instance.__send__(method_name, 1, 2.0.to_s, 3) }.must_raise ArgumentError
      end
      ...
      describe "when return type is wrong" do
        ...
        it "must raise" do
          _ { instance.__send__(method_name, 1, 2) }.must_raise TypeError
        end
      end
    ...
    describe "typeclass constrained method" do
      ...
      it "must succeed if types are correct and satisfy typeclass constraint" do
        _(instance.__send__(method_name, 1, 2.to_s)).must_equal 2.to_s + 1.show
      end

      it "must raise if types are incorrect" do
        _ { instance.__send__(method_name, 1, 2.0) }.must_raise TypeError
      end

      it "must raise if typeclass constraint is not satisfied" do
        _ { instance.__send__(method_name, 1.0, 2.0) }.must_raise TypeError
      end
    end
    ...
    describe "method with multiple typeclass constraints" do
      ...
      it "must succeed if all typeclass constraints are satisfied" do
        _(instance.__send__(method_name, Just["a"])).must_equal Maybe.return('"A"')
      end

      it "must raise if not all typeclass constraints are satisfied" do
        _ { instance.__send__(method_name, ["a"]) }.must_raise TypeError
      end
    end
    ...
    describe "method with typeclass constrainted return type" do
      ...
      it "must succeed if return typeclass constraint is satisfied" do
        _(instance.__send__(method_name, 1)).must_equal Either.return(1)
      end

      it "must raise if return typeclass constraint is not satisfied" do
        _ { instance.__send__(method_name, 2) }.must_raise TypeError
      end
    end
  end
  ...
  describe "def_typed_curried" do
    ...
    describe "when not all args passed it must check passed args and return proc" do
      ...
      it "must succeed if types are correct" do
        _(instance.__send__(method_name, 1)).must_be_instance_of Proc
      end

      it "must raise if types are incorrect" do
        _ { instance.__send__(method_name, 2.0) }.must_raise TypeError
      end
